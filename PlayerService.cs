...
internal class PlayerService
{
	...
	readonly Dictionary<FixedString64Bytes, PlayerData> namePlayerCache = new Dictionary<FixedString64Bytes, PlayerData>();
	readonly Dictionary<ulong, PlayerData> steamPlayerCache = new Dictionary<ulong, PlayerData>();
	readonly Dictionary<NetworkId, PlayerData> idPlayerCache = new Dictionary<NetworkId, PlayerData>();
	...
	internal PlayerService()
	{
		...
		if(!_loggedOnce)
		{
			...
			Core.Log.LogInfo($"Player cache initialized with {namePlayerCache.Count} entries; {onlinePlayers.Count()} online.");
			var block = string.Join("\n", onlinePlayers);
			if (!string.IsNullOrWhiteSpace(block)) Core.Log.LogInfo(block);
			_loggedOnce = true;
		}
	}
