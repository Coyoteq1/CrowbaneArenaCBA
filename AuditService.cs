...
internal class AuditService : IDisposable
{
	...
	public AuditService()
	{
		...
		global::CrowbanePackPlugin.CrowbanePackPlugin.HarmonyInstance.Patch(canCommandExecuteMethod, postfix: postfix);
	...
	static void HookUpForSeeingIfCheckingPermission()
	{
		...
		global::CrowbanePackPlugin.CrowbanePackPlugin.HarmonyInstance.Patch(executeCommandWithArgsMethod, prefix: prefixExecute, postfix: postfixExecute);
		...
		foreach (System.Collections.DictionaryEntry asmEntry in assemblyCommandMap)
		{
			if (asmEntry.Key is Assembly asm && asm.GetName().Name == "VampireCommandFramework")
			{
				...
				foreach (System.Collections.DictionaryEntry cmdEntry in commandDict)
				{
					...
					global::CrowbanePackPlugin.CrowbanePackPlugin.HarmonyInstance.Patch(methodInfo, prefix: prefixHelp, postfix: postfixHelp);
				}
			}
		}
	}
